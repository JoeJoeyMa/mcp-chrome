...
import ModelCacheManagement from './components/ModelCacheManagement.jsx';
import {
  DocumentIcon,
  DatabaseIcon,
  BoltIcon,
...
const App = () => {
  const [nativeConnectionStatus, setNativeConnectionStatus] = useState('unknown');
  ...
  const [modelInitializationStatus, setModelInitializationStatus] = useState('idle');
  ...
  const [semanticEngineStatus, setSemanticEngineStatus] = useState('idle');
  ...
  const [semanticEngineLastUpdated, setSemanticEngineLastUpdated] = useState(null);

  // Cache management
  const [isManagingCache, setIsManagingCache] = useState(false);
  const [cacheStats, setCacheStats] = useState(null);

  const statusMonitoringInterval = useRef(null);
  const semanticEngineStatusPollingInterval = useRef(null);
  ...
  const mcpConfigJson = JSON.stringify(
    {
      mcpServers: {
        'streamable-mcp-server': {
          ...
          url: `http://127.0.0.1:${serverStatus.port || nativeServerPort}/mcp`,
        },
      },
    },
    null,
    ...
    try {
      ...
      if (result.selectedModel) {
        ...
        if (PREDEFINED_MODELS[storedModel]) {
          ...
          console.log(`✅ Loaded valid model: ${currentModel}`);
        } else {
          console.warn(
  ...
  const saveVersionPreference = async (version) => {
    try {
  ...
  const savePortPreference = async (port) => {
    try {
  ...
  const refreshStorageStats = async () => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        console.log('✅ Storage stats refreshed:', storageStats);
      } else {
            ...
            {availableModels.map((model) => (
              <div
                ...
                className={`model-card ${
                  currentModel === model.preset ? 'selected' : ''
                } ${isModelSwitching || isModelDownloading ? 'disabled' : ''}`}
                onClick={() =>
                  !isModelSwitching && !isModelDownloading && switchModel(model.preset)
                }
              >
